...
try {
    ...
    if ($action === 'update_remark') {
        ...
        // 修正：將 WHERE id 改為 WHERE log_id，對應 SQL 中的欄位名
        $stmt = $conn->prepare("UPDATE check_in_logs SET remark = ? WHERE log_id = ?");
    ...
    if ($action === 'delete_person') {
        ...
        if ($member_id > 0 && $target_name !== '') {
            ...
            if ($type === 'parent') {
                ...
                if (($key = array_search($target_name, $parents)) !== false) {
                    unset($parents[$key]);
                }
    ...
    if ($action === 'final_checkin') {
        ...
        $display_name = $new_parent ?: "家長";
        if (empty($display_name) && !empty($selected_parents)) {
            $display_name = explode('|', $selected_parents)[0];
        }
   ...
   // --- 功能 C：電話查詢 ---
    ...
    // 💡 修正：刪除下方原有的重複註冊邏輯塊，統一由 action === 'register' 處理。
